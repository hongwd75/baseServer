...
namespace Project.Scheduler
{
	...
	public class SimpleScheduler
	{
		...
		{
			Interlocked.Exchange(ref NextDueTick, long.MaxValue);
		...
		private void SignalNextDueTick(long value)
		{
			...
			do
			{
				initialValue = Interlocked.Read(ref NextDueTick);
				if (value >= initialValue) return;
			}
			while (Interlocked.CompareExchange(ref NextDueTick, value, initialValue) != initialValue);
		...
		{
			Interlocked.Exchange(ref NextDueTick, long.MinValue);
